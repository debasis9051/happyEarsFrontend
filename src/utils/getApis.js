...
const getPatientList = async (currentUserInfo, svRef) => {
    axios.post(`${process.env.REACT_APP_BACKEND_ORIGIN}/get-patient-list`, { current_user_uid: currentUserInfo.uid, current_user_name: currentUserInfo.displayName }, { headers: { 'Content-Type': 'application/json' } })
        .then((res) => {
            if (res.data.operation === "success") {
                svRef(res.data.info)
            }
            else {
                console.log(res.data.message, "/get-patient-list");
            }
        })
        .catch((err) => {
            console.log(err)
